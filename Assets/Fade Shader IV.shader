Shader "Custom/Fade Shader IV"
{
	...
	SubShader{
		...
		{		
			...
			half rggradient = snoise(IN.worldPos.rg);
			half gbgradient = snoise (IN.worldPos.gb);
			clip(rggradient- _DissolvePercentage);
			clip(gbgradient- _DissolvePercentage);
			half totalGradient = rggradient * gbgradient;

			fixed4 c = lerp(1, totalGradient, _ShowTexture) * _Color;
			o.Albedo = c.rgb;
